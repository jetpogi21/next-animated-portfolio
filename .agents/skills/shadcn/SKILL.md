...
- **`components.json`** is at the **repository root**. Run **`npx shadcn@latest …`** from the root; generated primitives land under **`src/components/ui/`** (see **`components.json`** → **`aliases`**). See **`.agents/skills/galaxy-monorepo/SKILL.md`** for overall **`src/`** layout.
...
5. **Respect screen real estate.** Especially inside **admin CRUD dialogs** and dense forms: keep triggers compact (`h-7` / `text-xs`, tight `py`/`gap`), and put long pickers or searchable lists in **`Popover`**, **`DropdownMenu`**, or **`Command`** (portaled overlays) instead of expanding **in-flow** panels that steal height from siblings. Pair with **`max-h-*` + `overflow-y-auto`** on list bodies.
