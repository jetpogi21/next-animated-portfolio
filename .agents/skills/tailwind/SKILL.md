...
## Screen real estate (dense UI)

- **Treat vertical space as scarce** in dialogs, side panels, and multi-section forms: prefer **small** controls (`h-7`/`h-8`, `text-xs`, `gap-1`/`gap-1.5`, `py-1`/`py-1.5`) and **short** trigger rows over large padded blocks.
- **Long lists** belong in a **scrollable** region (`max-h-56 min-h-0 overflow-y-auto` or similar), often inside a **portaled** overlay (shadcn **`Popover`**, **`DropdownMenu`**) so they do not push the rest of the form down the page.
- **Flex siblings**: avoid a single section growing without bound (`flex-1` on a growing in-flow list); cap height or use an overlay so **note fields**, totals, and actions stay reachable without excessive scrolling.
